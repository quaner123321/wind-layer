...
import vert from '../../../shaders/num.vert.glsl';
import frag from '../../../shaders/num.frag.glsl';
...
export interface NumPassOptions {
  source: SourceType;
  texture: Texture;
  textureNext: Texture;
  bandType: BandType;
 ...
 * number
 */
export default class NumPass extends Pass<NumPassOptions> {
  #mesh: WithNull<Mesh>;
  ...
  constructor(id: string, renderer: Renderer, options: NumPassOptions = {} as NumPassOptions) {
  ...
  render(rendererParams, rendererState) {
    ...
    if (rendererState && this.#mesh && tiles && tiles.length > 0) {
      ...
      for (let i = 0; i < tiles.length; i++) {
        ...
        // 在render方法中传递字体大小
        this.#mesh.program.setUniform('fontSize', rendererState.symbolSize ? rendererState.symbolSize[0] * 0.01 : 1.5);
